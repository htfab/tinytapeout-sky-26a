...
The design contains 32 ring oscillators instantiated from the SKY130 cell library. When the device is triggered, it performs all (32C2) = 496 pairwise comparison by feeding the selected oscillators into a counter. For each RO i, the FSM counts the number of later ROs (j>i) that it beat, and the count is stored as a 129-bit Lehmer code.
...
5) Read the result: drive CD low, and clock 129 bits out with MISO and SCK. MISO shifts out the MSB first.
